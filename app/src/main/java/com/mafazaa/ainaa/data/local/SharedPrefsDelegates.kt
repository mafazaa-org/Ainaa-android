 ...
 */
val SharedPreferences.delegates get() = SharedPrefsDelegates(this)

class SharedPrefsDelegates(private val prefs: SharedPreferences) {
    ...
    fun intPair(
        default: Pair<Int, Int> = 0 to 0,
        key: String? = null,
    ): ReadWriteProperty<Any, Pair<Int, Int>> =
        create(default, key, { k, d ->
            val first = prefs.getString(k + "_first", d.first.toString())?.toInt()!!
            val second = prefs.getString(k + "_second", d.second.toString())?.toInt()!!
            first to second
        }, { k, v ->
            prefs.edit().putString(k + "_first", v.first.toString())
                .putString(k + "_second", v.second.toString())
        })
